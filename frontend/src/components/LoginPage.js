...
export default function LoginPage() {
  ...
  function loginUser() {
    ...
    axios.post("http://localhost:9000/api/invManager/login", data).then((res) => {
      if(res.status == 200) {
        if(res.data.type == 'InvM') {
          window.location.href="/dashboard"
        }
        else if(res.data.type == 'Cust') {
          window.location.href="/allProducts"
        }
        else if(res.data.type == 'boat') {
          window.location.href="/bownerdashboard"
        }
      }
      else {
        swal({
        ...
        <div class="container-fluid">
          <div class="row">
            ...
            <div class="col-sm-6 px-0 d-none d-sm-block">
              <img
                src="https://www.thaiunion.com/files/blog/sustainability/fish.jpg"
                alt="Login image"
                class="w-100 vh-100"
